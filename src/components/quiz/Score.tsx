...
const Score = ({ questions, score, handleRetry, handleNewQuiz }: Props) => {
  ...
  const questionLabel = total - score === 1 ? "question" : "questions";

  return (
    <>
      <div className="grid gap-8 items-center justify-center px-4">
        <div className="text-center">
          <div className="text-2xl sm:text-4xl mb-3">{scorePercentage > 65 ? "Congrats! 🎉" : "Nice try! 🤪"}</div>
          <div className="text-4xl sm:text-7xl font-bold mb-6">
            <span className="text-purple-600">{scorePercentage}</span>%
          </div>
          <div>
            You missed {total - score} {questionLabel}.
          </div>
        </div>

        {questions.map((item) => (
          <AnsweredQuestion key={item.question} item={item} />
        ))}
...
function AnsweredQuestion({ item }: { item: SanitizedQuestion }) {
  const answerColor = item.correctAnswer === item.selectedAnswer ? "text-green-600" : "text-red-600";
    ...
    <div>
      ...
      <div className="px-3">
        <div className="flex items-center gap-3">
          <span>Selected:</span>
          <span className={`font-bold ${answerColor}`} dangerouslySetInnerHTML={{ __html: domPurify.sanitize(item.answers[item.selectedAnswer]) }} />
        </div>
        <div className="flex items-center gap-3">
          <span>Correct:</span>
